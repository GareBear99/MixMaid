# MixMaid — Technical Audit

Scope: MixMaid release baseline. This audit mirrors the FreeEQ8 / MeterMaid
structure so readers can compare plugins apples-to-astructure sSource layout
- `MixMaid*/Source/PluginProcessor.{h,cpp}` — audio processor + parameter layout
- `MixMaid*/Source/PluginEditor.{h,cpp}` — custom-painted UI
- `MixMaid*/CMakeLists.txt` — JUCE plugin build
...
## Standards
MixMaid is built with JUCE 8.0.0 (C++20) and targets VST3 / AU / Standalone
